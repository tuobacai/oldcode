...
class SimSiamese(object):
    ...
    def __init__(self,config,is_training=True):
        ...
        with tf.name_scope('loss'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits + 1e-10,labels=self.y_data)
    ...
    def lstm_encoder(self):
        with tf.variable_scope('bilstm'):
            ...
            outputs_x1, _ = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, self.inputs_x1,dtype=tf.float32)
            ## 开启变量重用的开关
            tf.get_variable_scope().reuse_variables()
            outputs_x2, _ = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, self.inputs_x2,dtype=tf.float32)

        # with tf.variable_scope('max pooling layer'):
    ...
    def feature_match(self):
        ...
        product_fea = self.output_x1_pooled*self.output_x2_pooled
        sub_fea = tf.abs(self.output_x1_pooled - self.output_x2_pooled)
        res_fea = tf.concat([self.output_x1_pooled, self.output_x2_pooled, sub_fea, product_fea],axis=1)
        return res_fea

    '''mlp输出层'''
    def mlp_classifier(self):
        with tf.variable_scope('mlp_layer'):
            input_w = tf.get_variable(name='input_w', shape=[self.config.hidden_size * 2*4, self.config.fc_dim],
                                      dtype=tf.float32)
    ...
    '''双向lstm'''
    def bi_lstm(self):
        lstm_fw_cell = rnn.BasicLSTMCell(self.config.hidden_size)  # forward direction cell
        lstm_bw_cell = rnn.BasicLSTMCell(self.config.hidden_size)  # backward direction cell
        if self.config.dropout_keep_prob is not None:
            lstm_fw_cell = rnn.DropoutWrapper(lstm_fw_cell, output_keep_prob=self.config.dropout_keep_prob)
            lstm_bw_cell == rnn.DropoutWrapper(lstm_bw_cell, output_keep_prob=self.config.dropout_keep_prob)

        return lstm_fw_cell, lstm_fw_cell
