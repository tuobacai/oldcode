...
import os
from datautils import DataUitls
...
flags=tf.app.flags
FLAGS=flags.FLAGS
flags.DEFINE_integer('batch_size',64,'the batch size of the training procedure')
...
class ModelTrain(object):
    ...
    def train_step(self):
        ...
        train_data, valid_data, test_data = self.utils.load_data(FLAGS.max_len, batch_size=config.batch_size)
        ...
        with tf.Graph().as_default(), tf.Session() as session:
            ...
            for i in range(config.num_epoch):
                ...
                global_steps = self.run_epoch(train_model, session, train_data, global_steps, valid_model, valid_data,
    ...
    def run_epoch(self,model,session,data,global_steps,valid_model,valid_data,train_summary_writer,valid_summary_writer=None):
        for step,(x,y,mask_x) in enumerate(self.utils.batch_iter(data,batch_size=self.config.batch_size)):
            feed_dict={}
            ...
            valid_accuracy =self.evaluate(valid_model,session,valid_data,global_steps,valid_summary_writer)
    ...
    def evaluate(self,model, session, data, global_steps=None, summary_writer=None):
        correct_num = 0
        total_num = len(data[0])
        for step, (x, y, mask_x) in enumerate(self.utils.batch_iter(data, batch_size=FLAGS.batch_size)):
